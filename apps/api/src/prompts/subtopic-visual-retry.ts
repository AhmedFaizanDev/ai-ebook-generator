export function buildVisualRetryPrompt(subtopicTitle: string, isTechnical: boolean = true): string {
  const codeBlockInstruction = isTechnical
    ? 'Use code blocks only for programming/software/CS topics — not for finance, economics, or business analysis (use prose and tables). Do not use raw HTML. Close all code blocks with ```.'
    : 'Do NOT include code blocks, raw HTML, or markup. Use tables and prose only.';
