...
import { structureJsonLanguageRule } from '@/lib/output-language';
...
export function buildStructurePrompt(
  ...
  const titleGuidance = isTechnical
    ? 'Unit and subtopic titles: specific and technical (e.g. "Request lifecycle" not "Introduction").'
    : 'Unit and subtopic titles: specific and substantive (e.g. "The Hero\'s Journey" not "Introduction"). Titles should reflect the narrative, conceptual, or thematic content of the topic.';

  const langLine = structureJsonLanguageRule(outputLanguage);
...
Required counts (do not deviate): Exactly ${UNIT_COUNT} units. Each unit has exactly ${SUBTOPICS_PER_UNIT} subtopics (${SUBTOPICS_PER_UNIT} strings in "subtopics" array). Exactly ${CAPSTONE_COUNT} capstone topic${CAPSTONE_COUNT > 1 ? 's' : ''}. Exactly ${CASE_STUDY_COUNT} case study topic${CASE_STUDY_COUNT > 1 ? 's' : ''}. Every title and topic must be a non-empty string. ${titleGuidance} Order units foundational to advanced. No duplicate subtopic titles.${langLine ? ` ${langLine}` : ''}`;
}
