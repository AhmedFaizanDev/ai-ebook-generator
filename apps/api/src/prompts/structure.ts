...
export function buildStructurePrompt(topic: string, isTechnical: boolean = true): string {
...
Required counts (do not deviate): Exactly ${UNIT_COUNT} units. Each unit has exactly ${SUBTOPICS_PER_UNIT} subtopics (${SUBTOPICS_PER_UNIT} strings in "subtopics" array). Exactly ${CAPSTONE_COUNT} capstone topic${CAPSTONE_COUNT > 1 ? 's' : ''}. Exactly ${CASE_STUDY_COUNT} case study topic${CASE_STUDY_COUNT > 1 ? 's' : ''}. Every title and topic must be a non-empty string. ${titleGuidance} Order units foundational to advanced. No duplicate subtopic titles.`;
}
